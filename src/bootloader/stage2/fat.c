...
uint FAT_Initialize(DISK* disk) {
    ...
    err = DISK_ReadSectors(disk, rootDirLba, 1, g_Data->RootDirectory.Buffer);
    if (err) {
...
FAT_File far* FAT_OpenEntry(DISK* disk, FAT_DirectoryEntry* entry) {
    ...
    uint err = DISK_ReadSectors(disk, FAT_ClusterToLba(fd->CurrentCluster), 1, fd->Buffer);
    if (err) {
    ...
    while (byteCount > 0) {
        ...
        if (leftInBuffer == take) {
            ...
            if (fd->Public.Handle == ROOT_DIRECTORY_HANDLE) {
                ...
                // read next sector
                uint err = DISK_ReadSectors(disk, fd->CurrentCluster, 1, fd->Buffer);
                if (err) {
                    printf("FAT: read error!\r\n");
                    break;
                }
                ...
                // read next sector
                uint err = DISK_ReadSectors(disk, FAT_ClusterToLba(fd->CurrentCluster) + fd->CurrentSectorInCluster, 1, fd->Buffer);
                if (err) {
                    printf("FAT: read error!\r\n");
                    break;
                }
...
uint FAT_FindFile(DISK* disk, FAT_File far* file, const char* name, FAT_DirectoryEntry* entryOut) {
    char fatName[12];
...
FAT_File far* FAT_Open(DISK* disk, const char* path) {
    ...
    while (*path) {
        ...
        FAT_DirectoryEntry entry;
        uint err = FAT_FindFile(disk, current, name, &entry);
        if (err) {
